...
class SEInception3(nn.Module):
    ...
    def __init__(self,  num_classes=1000):
        ...
        # initialize the weight in SE blocks
        for m in self.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
                import scipy.stats as stats
                stddev = m.stddev if hasattr(m, 'stddev') else 0.1
                initial_weight = stats.truncnorm(-2, 2, scale=stddev)
                values = torch.Tensor(initial_weight.rvs(m.weight.data.numel()))
...
if __name__ == '__main__':
    ...
    def test_net():
        ...
        optimizer = SGD(filter(lambda p: p.requires_grad, net.parameters()), lr=0.01, momentum=0.9, weight_decay=0.0001)
        optimizer.zero_grad()
